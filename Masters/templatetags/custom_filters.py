...
@register.filter
def add(value, arg):
    """Adds the given argument to the value."""
    try:
...
def enumerate_list(value):
    """Returns an enumerated list [(index, item), (index, item), ...]"""
    return list(enumerate(value))

@register.filter
def enumerate_dict(value):
    """Returns an enumerated dictionary [(index, key, value), ...]"""
    return [(index, key, value) for index, (key, value) in enumerate(value.items())]